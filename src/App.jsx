...
function App() {
  ...
  return (
    <div className="App container mx-auto sm:w-3/4 lg:w-1/2">
      <div className="card">
        <ul className='my-5'>
          {
            bikes.length ?
            (bikes.map(bike => (
              <li className={` ${(!bike.completed) ? 'bg-gray-200' : 'bg-green-400'} rounded-full px-4  flex flex-row justify-between my-3`} key={bike.id}>
